...
class W90:
    ...
    def run(self):
        ...
        if self.U_matrix_opt.shape[-1] == self.num_bands_tot:
            for k in range(self.num_kpts_loc):
                n = np.where(self.lwindow[k] == True)[0][0]
                self.U_matrix_opt[k] = np.roll(self.U_matrix_opt[k], n, axis=-1)
        # else:
        #     U_temp = np.zeros([self.num_kpts_loc, self.num_wann, self.num_bands_tot], dtype = np.complex128)
        #     U_temp = U_temp[:,:,np.array(self.band_included_list)] + self.U_matrix_opt
        #     self.U_matrix_opt = U_temp
    ...
    def export_AME(self, grid=[50, 50, 50], filename="wannier90"):
        """
    ...
    def get_wannier(self, supercell=[1, 1, 1], grid=[50, 50, 50]):
        ...
        for k_id in range(self.num_kpts_loc):
            ...
            C_tilde = C_opt.dot(self.U_matrix[k_id].T)
            kpt = kpts[k_id]
            ao = numint.eval_ao(self.cell, grids_coor, kpt=kpt)
            u_ao = lib.einsum("x,xi->xi", np.exp(-1j * np.dot(grids_coor, kpt)), ao)
            u_mo.append(lib.einsum("xi,in->xn", u_ao, C_tilde))
        ...
        try:
            for WF_id in range(self.num_wann_loc):
                ratio_max = np.abs(
                    WF0[np.abs(WF0[:, WF_id].real) >= 0.001, WF_id].imag
                    / WF0[np.abs(WF0[:, WF_id].real) >= 0.001, WF_id].real
                ).max(axis=0)
                print(
                    "The maximum imag/real for wannier function ",
                    WF_id,
                    " : ",
                    ratio_max,
                )
        except:
            print("Wanring: divide by zero encountered")
        return WF0
    ...
    """
    def get_wanproj(self):
        R = np.array(self.U_matrix)
        with open('wanproj.txt', 'w') as outfile:
        ...
        np.savetxt('wanproj', R, delimiter=',')
    """

    def get_wanproj_array(self):
        return np.array(self.U_matrix)

    def get_wanproj_array_opt(self):
        return np.array(self.U_matrix_opt)

    def WANPROJ(self, filename="WANPROJ"):
        arr = np.array(self.U_matrix)
        np.save(filename + ".npy", arr)
        arr_reshaped = arr.reshape(arr.shape[0], -1)

        # saving reshaped array to file.
        np.savetxt(filename + ".txt", arr_reshaped.view(float))

        arr_opt = np.array(self.U_matrix_opt)
        arr_opt_reshaped = arr_opt.reshape(arr_opt.shape[0], -1)

        # saving reshaped array to file.
        np.savetxt(filename + "_opt.txt", arr_opt_reshaped.view(float))
...
if __name__ == "__main__":
    ...
    import PyFoldHub.pywannier90_s as pywannier90_s
    ...
    """
    w90 = pywannier90_s.W90(kmf, cell, kmesh, num_wann, other_keywords=keywords)
    w90.kernel()